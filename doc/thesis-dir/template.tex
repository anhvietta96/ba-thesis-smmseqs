...
type & \(q\) & Number & Score matrix \\
               &       & of \Qgram{q}s & entries\\
\hline
\(q\)-grams  & 2 & 400 & \numprint{160000}\\
...
the score matrix is calculated as the square of the number of (sorted) \(q\)-grams.}
\label{tab:numdiff}
\end{center}
\end{table}

\SKcomment{Note that I have corrected the first number of entries for \(q=3\)
in Table \ref{tab:numdiff}. I have also reordered the lines as one
wants to compare the values for the same value of \(q\).}
...
\SKcomment{You should merge the first two lines of initializing
\(\LE{q}{\Alpha}\) with the the first element: Just assign a list with a
single elements to \(\LE{q}{\Alpha}\).}
...
\SKcomment{You better use the section header 'determining the score-threshold'.
...
this probability. The subsection then describes your approach. In this latter
...
is performed, where the number of prefiltering operations
\begin{align}
...
distributed \Qgram{7}. The blue bars show the frequencies for events with
...
determined a sample \(Q\) of all \(q\)-grams for \(q=7\) and then computed
the score for each \((u,u')\in Q\times Q\), \(u<u'\)?
How large was \(Q\)? Did you specify which score function you use?}
...
the second is based on an unkown probability function. Please be more precise.
...
distributed above, which refers to a sampling procedure. So you actually have
not sampled the \(q\)-grams and computed the exact value? Actually, I cannot
tell from you sloppy description.}

\SKcomment{Please explain, why Equation \ref{equation:convolution} is based
on convolution. Maybe a citation?}
...
large \(q\), since a \(q\times q\) score matrix

\SKcomment{For \(q=7\) such a matrix consists of \(q^{2}=49\) elements.
You likely refer to the \(r^{q}\times r^{q}\)-matrix which has
\(r^{2q}\) entries.}
...
\SKcomment{Why are the \Qgram{2} and \Qgram{3} matrices are evaluated.
They have been computed at compile time, i.e.\ for each \(2\)-gram and
\(3\)-gram \(u\) one can obtain the \(q\)-gram/score-pairs \((v,\sigma(u,v))\)
in sorted order.}
