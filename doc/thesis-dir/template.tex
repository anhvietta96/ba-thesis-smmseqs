...
of hash values in every sequence position must be prioritized. In order to
...
If the spaced seed can be maximally divided into \(b\) blocks of consecutive
...
the seed into shorter blocks of consecutive 1s, intertwined by blocks of
...
and a recursive hashing function.
...
The methods described above were implemented in a program called SMMseqs.
The implementation in C++ follows an object-oriented approach and
...
\Qgram{q}s/sorted \Qgram{q}s sequentially.
...
based on Algorithm~\ref{code:mmseqshash}. Additionally, it is also equipped with
an implementation of insertion sort to sort and hash sequence data to provide
encodings of sorted \Qgram{q}s.
\end{enumerate}
...
\item A recursive hash engine to collect \Qgram{q} hash from sequences.
...
unsorted \Qgram{q}s.
\item An environment constructor gets encoding of sorted \Qgram{q}s and uses
them to call individual scores from the score
...
\AVTcomment{I mean the score table \(\Scoretable{q}{u}\) for all \(u\) that is
sorted \Qgram{q}}
...
\AVTcomment{I did not use recursive hashing for query processing since it needs
sorting the extracted \Qgram{q}s. Anyway I changed the classification to basing
...
\AVTcomment{I think dependency is a better word. I've changed accordingly.}
...
\AVTcomment{I'm adding legend to the figure.}
...
linear encoding table LE (see Algorithm~\ref{code:linearEncodingTable}) depends
only on the index tables of sorted \Qgram{q}s and the scoring function, it also
can be evaluated at compilation.
...
\AVTcomment{There is no sorting here, I hope this leads to less confusion.}
...
Using the recursive hashing seed reader, the target
sequences are then hashed in \(O(nb)\) time and the hashes are packaged as
byte units and saved in a dynamically-sized contiguous container (i.e.\ vector).
...
in sorted order.}
\AVTcomment{They are not computed in compile time, please see
Section~\ref{section:scorematrix}}
...
seed of weight 4, only the \Qgram{2} score matrix is created. The entry in the
score matrix is represented as a pair of score and unsorted \Qgram{q} code and
sorted after score value, resulting in the score tables \(\Scoretable{q}{u}\) as
input data vectors for the \(k\)-enviroment construction stage.
...
The second sum involves the current region in the query sequence and
...
\(k\)-environment. I would keep the abtract view which only requires
...
target, on the query, the diagonal number as the difference between the target
...
\SKcomment{What do you mean by similar \Qgram{q} generation? Is this the
construction of the \(k\)-environments?}

All programs considered here were compiled with
\mbox{g++ 11.3.0}. The measurement were performed on an Intel CPU
i5-\numprint{10300}H at 2.5~GHz clock rate with 8~GB DDR4 RAM
running Ubuntu on an SSD.

\SKcomment{The fact that all programs considered here were written in C++
needs to be mentioned in the implementation section. I had suggested some
wording in a comment. To remove redundancies, I modified the previous
sentence.}
...
consists of \numprint{20000} sequences of length between 7 and \numprint{8081}.
...
\texttt{DB.fasta}. This file of \numprint{426} sequences of length between
8 and \numprint{4291} serves as query sequence file.
\end{itemize}
Some statistics on these datasets are summarized below in
Table~\ref{tab:datasets}.

\SKcomment{Table \ref{tab:datasets} provided little information. So it is
better to add this information to the text and remove the table.}
...
performance between the hashing approaches, tests were carried out on randomly
generated dataset consisting of \numprint{50000000} sequence characters.

\SKcomment{Are these characters stored in one sequence, or multiple sequences?}
...
\SKcomment{Table \ref{tab:seeds} show the theoretical speed up factors for some
spaced seeds of weight \(q\leq 7\). But the speedup factor
are partly smaller than 1 and partly larger than 1. Also the seeds are
different from those in the Table \ref{tab:seeds}. So I do not how you
deduce the expection?}

\SKcomment{It is superfluous to number the seeds if these numbers are not
used. This also holds for Table \ref{tab:seeds}. Only for the seeds
in \ref{tab:asymptoticseed9} and \ref{fig:asymptoticseed9} you refer to the
seeds by their number.}

Even the hashing using seed \numprint{111011}, which
...
\numprint{50000000} character with default MMseqs2 spaced seeds}
\label{fig:target_hashing}
\end{figure}

To examine the asymptotic behavior

\SKcomment{By your experiments you do not examine the asymptotic behavior.
You just compare the asymptotic behavior with empirical results.}

of the two hashing algorithms, seeds of
weight 9 and 12 were further tested. For each seed weight \(q\), the number of
seed blocks ranges from \(1\) to \(\frac{q}{2}-1\). The generated seeds are
characterized in Table~\ref{tab:asymptoticseed9} and \ref{tab:asymptoticseed12}.
The test results are summarized in Figure~\ref{fig:asymptoticseed9} and
\ref{fig:asymptoticseed12}.
...
\end{center}
\caption{Performance of two hashing functions on randomly generated dataset of
\numprint{50000000} characters with seeds in Table~\ref{tab:asymptoticseed9}.}
...
\end{center}
\caption{Performance of two hashing functions on randomly generated dataset of
\numprint{50000000} characters with seeds in Table~\ref{tab:asymptoticseed12}.}
...
\SKcomment{I am really surprised that recursive hashing only gives such a
small speedup for seeds consisting only of 1 block. One would need to have a
look at the code to understand this, but this is not a task here. After all,
the absolute times are always about 0.5s, which hardly has an influence on the
...
We tested the context-free and the context-sensitive approach to calculate
...
\(k\)-environment per position and MMseqs2 result.

\SKcomment{Unclear what you mean by 'and MMseqs2 result'. So the
MMseqs2 result is the third approach to estimate the number of
\(q\)-grams? Has this result been
obtained by running MMseqs2 or is it the result you have obtained by your
implementation of the MMseqs2 method?}

\SKcomment{It is generally good practice to refer to the methods
considered here by providing the section number in which they are
described.}
...
\SKcomment{In the figure you need to explain that the different
number refer to the differnt sensitivity value.}

\SKcomment{Note that I have revised the caption to clarify what is shown. Please
adjust the other captions accordingy.}
...
control compared to MMseqs2

\SKcomment{You need to explain what you mean by better control.}

\SKcomment{Where are the results for MMseqs2?}
...
\SKcomment{Why should this be a reason. The sub-\Qgram{q} length is only
relevant for the the context-sensitive approach.}

Since the dividing schema of the branch-and-bound algorithm is reused,

\SKcomment{Why do you mention the branch-and-bound algorithm? Before you have
mentioned that the method is the context-sensitive method.}
...
\SKcomment{Sorry, as I do not fully understand this argument,
as the context-free approach is not clear to me.}
...
the overhead caused by the sorting and the undoing of the sorting of
\Qgram{q}s.

\SKcomment{I doubt that the sorting is problematic, as you have to sort only
once. Undoing the sorting has to be done for each generated \(q\)-grams, so this
may be a problem. A point of future work would be to really measure the
contribution of undoing the sorting. This could easily be done by only
collecting the \(k\)-environments without processing them further. One version
would apply the sorting, the other not. The difference would give the
...
score matrices), compared to \numprint{1728}~MB for the
...
\SKcomment{The problem with \(q>7\) is the fact that the number of
possible \(q\)-grams becomes so large. So if we would construct the
same number of \(q\)-grams in about the same runtime, it would be a much
smaller fraction of the entire search space. But lets see. Maybe this is
feasible with the appropriate threshold.}
...
In Figure, the recorded time in each step of the program is visualized.

\SKcomment{Add reference to figure in previous sentence.}
\begin{figure}
\centering
...
of weight 6 to the seed \numprint{1111010101} of weight 7.
...
\SKcomment{Why do these results appear here and not in the results section?}
...
\SKcomment{please add date for \ldots}

\SKcomment{In previous experiment you showed that the method using
sorted \(q\)-grams is slower than the method using \(q\)-grams. But here
the result it different. Likely, I am missing something.}
...
calculation option, ranging from MMseqs2 mode
to new sorted \Qgram{q}

\SKcomment{As far as I understand, MMseqs2 mode refers to the way to calculate
\(k\) from a given sensitivity setting. But the new sorted \(q\)-gram
mode refers to the way of first precomputing score tables for sorted
\(q'\)-grams, using these to create sorted \(q\)-grams and finally undoing the
sorting. So it is strange that you use
'from MMseqs2 mode new sorted \(q\)-gram mode' in one sentence.
I actually do not think that you need to mention the different variants
of the implementation here. It would suffice to state:
The program provides options to select one of three implemented methods to
determine the score threshold \(k\) and one of the two methods to
construct the \(k\)-environment.}
...
Earlier it was shown that the runtime of \Qgram{q} generation scales
almost constantly to seed, therefore some testing was created to test for
seeds of weight 8 and 9:

\SKcomment{Unclear what you mean by 'scales almost constantly to seed'.
You likely want to state: \ldots the average runtime for generating
a \Qgram{q} of the \(k\)-environment hardly increases with an increasing
weight of the seed. So some experiments using the weight 8 and 9 for
the seeds were generated.}

\AVTcomment{Waiting for BytesUnit fix}

\SKcomment{Unclear which Bug/missing feature you are referring to.}
...
\SKcomment{How can an implementation decide?}
...
\SKcomment{There are ways of sorting at compile time, see e.g.\
\url{https://stackoverflow.com/questions/32660523/c-calculate-and-sort-vector-at-compile-time}, but I think it is more a matter of efficient compile
time sorting.}

A manual compile time sorting implementation (i.e.\ quicksort)
would again demand more compile time instances.

\SKcomment{What do you mean by 'manual time sorting implementation'?}
...
\SKcomment{But you only need the sorted-sorted \(q\)-grams matrix. At least in
a previous table you had compared the sizes of sorted/sorted score matrix
with the unsorted-unsorted score matrix. So why would you consider
the sorted-unsorted score matrix? Am I missing something?}
\begin{figure}
\centering
...
possible improvement was a blockwise processing of all \(q\)-grams
...
was seen in the experiment reported here. A
new method of estimating the score threshold consistent with the number
of generated \Qgram{q}s was implemented and showed to be a better estimation
approach, both for low and high sensitivity settings. Finally, an optimized
