...
\EndIf
\EndFor
\State \(\LE{q}{\Alpha}.\mathsf{insert}(0,\CurrentWeight)\)\Comment{Prepend new weights at beginning}
\EndFor
\end{algorithmic}
\end{algorithm}
...
\subsection{A more accurate, context-sensitive estimation}
The above strategy treats single character as
independent and therefore loses its context in \Qgram{q}s, i.e what comes
before
and after. Another approach that perfectly preserves context could then be 
hashing the target sequences to
...
\State \(h \gets h_{\text{temp}}\)
\EndFor
\State \(\Cutoff \gets \frac{s}{r^q}\)\Comment{Evaluate \(k\) based on Equation~\ref{eq:distribution}}
\State \(i \gets \PossibleValuesTotal - 1\)
\State \(\Psum \gets 0\)
\While{\(i\neq 0\)}
\State \(\Psum\text{ += }h[i]\)
...
a C++ program called SMMseqs, which makes extensive use of the template system, 
with intention on scalability and high level of generalization.
...
using seed~\numprint{11101101}, sensitivity parameter \(s\in\{10,100,\numprint{1000}\}\)}
...
using seed \numprint{1111010101}, sensitivity parameter \(s\in\{10,100,\numprint{1000}\}\)}
