...
as section heading. Also MMseqs2 is not an own chapter, but a section of the
introduction.}
...
For every alphabet \(\Alpha\), we can define a bijective mapping to a subset
...
a hash value for each \Qgram{q} using \(O(q)\) time. For a sequence of length
\(n\), this would sum up to \(O((n-q+1)q)=O(nq)\).
...
\SKcomment{Unclear what you by this 'family of hash functions'? You likely mean incremental/recursive hash functions?}
...
provide the basic schema of recursive hashing. In an implementation, the most
...
\SKcomment{While you provide a detailed description of existing methods, your
...
involving spaced seeds need \(O(nq)\) time, while your block based method needs
\(O(nb)\) time, where \(b\leq q\). So your method reduces the runtime by
a factor of \(\frac{q}{b}\). If \(b=q\), there would be no speedup. The case
...
\SKcomment{Do you have a table with the spaced seeds, including the number of
...
\SKcomment{Omit the part beginning with structures. This is general bla bla,
not relevant here.}
...
substitution matrices widely used in bioinformatics.
...
in the GSA-lecture notes. Based on this, you explain the the branch-and-bound
...
decomposes \(u\) into groups of \Qgram{2}s and \Qgram{3}s \(u_i\),
prioritizing \Qgram{3}. For \(q\in \{2,3\}\), a score matrix

\SKcomment{Do not reuse the identifier \(u\) below, as it was introduced
as \(q\)-gram above. Also use \(q'\in\{2,3]\), as otherwise you cannot refer to the original \(q\)-gram.}

\SKcomment{Why prioritize \(q'=3\)? Does this mean that for \(q=4\) you split
it into a single character and a \(3\)-gram? You likely are only referring to
the method for \(q\geq 5\).}

\(\Scoretable{q}{u}=\lbrack (v,\sigma(u,v))\mid v\in\Alpha^{q}\rbrack\) is
computed and sorted by the the scores \(\sigma(u,v))\).
The full $k$-environment of a \Qgram{q} \(u\) can then be constructed from a
...
actually need to define first is the splitting of \(q\in\{4,\ldots,7\}\) into
...
for all \(u'\in\Alpha^{q'}\), \(\Scoretable{q'}{u'}\) is the sequence of pairs
...
\SKcomment{You should now provide an example with numbers of the size of the
scoretables for \(q'\in\{2,3\}\) which need to be precomputed. I assume that
you compute them without considering the score threshold \(k\) which only has
an effect when enumerating the cartesian product.}

\SKcomment{Also provide an example showing that the score of two \(q\)-grams
...
\(\varphi:\{0,1,\ldots,q-1\}\to\{0,1,\ldots,q-1\}\) be a bijective mapping and
define
\begin{align}
...
For all \(q\)-permutations \(\varphi\) and all \(u,v\in\Alpha^{q}\) it holds:
\begin{align}
...
\sigma(\varphi(u),\varphi(v))
  &= \sum_{j=0}^{q-1} \sigma (\Subchar{\varphi(u)}{j},
                              \Subchar{\varphi(v)}{j})\\
  &= \sum_{j=0}^{q-1} \sigma (\Subchar{u}{\varphi(j)},\Subchar{v}{\varphi(j)})\label{eq:jPerm}\\
  &= \sum_{i=0}^{q-1} \sigma (\Subchar{u}{i},\Subchar{v}{i})\label{eq:iPerm}\\
  &= \sigma(u,v)
\end{align}
...
\begin{align}
\Subchar{u}{\Perm{u}{i}}\leq\Subchar{u}{\Perm{u}{i+1}}\text{ for all }i, 0\leq
i\leq q-2
\end{align}
...
\SKcomment{I would not mention SIMD here, as this is an implementation issue,
