...
\SKcomment{The algorithm uses a value \(k\) which is not defined. You likely means the space of the seed.}
...
No & Seed & Span & Weight \(q\) & Number of Blocks \(b\) & Speedup in
hashing
\(=\frac{q}{2b}\)\\
\hline
...
algorithm considers only single character \(u[i]\) for \(0\leq i<q\)

\SKcomment{Note that you have used indexing from 1 in other context. Because always use the same indexing. I would prefer from 0.}

and
construction a
score table for each character \(\Scoretable{1}{u[i]}\). Then the
\(k\)-environment is contructed as a subset of the cartesian product
\(\bigtimes_{i=1}^{q} \Scoretable{|u_i|}{u_i}\):
\begin{align}
\text{Env}_k(u) = \{(v,\sigma(u,v))\mid(v,\sigma(u,v))\in \bigtimes_{i=1}^{q}
