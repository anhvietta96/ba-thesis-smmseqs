...
\(r^{q-1-i}\), where \(r\) is an integer called the radix and \(i\) is the position of the
...
%of hash values in every sequence position must be prioritized. 
In order to
...
\State \(\HashValVec\gets []\)
\State \(\HashValue \gets 0\)
\For{\(i\) \textbf{from} 0 to \(q-1\)}\Comment{Compute 1st hash}
\State \(\HashValue \gets \HashValue \cdot r\)
\State \(\HashValue \gets \HashValue + s[i]\)
\EndFor
\State \(\HashValVec .\Append(\HashValue)\)
\For{\(j\) \textbf{from} 0 to $n-q-1$}\Comment{Compute other hash values}
...
\(\PosToRmv:\Start\),\\
\(\WeightToRmv:r^{\Rank+i-1-\Start}\),\\
\(\PosToAdd:i\),\\
\(\WeightToAdd:r^{\Rank}\)\end{tabular}\})\)
\EndIf
\EndIf
\If{\(\Seed[i]\)}
\State \(\Rank\text{ -= }1\)
...
\State \(\Blocks.\mathsf{append}(\{\begin{tabular}{c}
\(\PosToRmv:\Start\),\\
\(\WeightToRmv:r^{\Rank+\Seed.\Span-1-\Start}=r^{\Seed.\Span-1-\Start}\),\\
\(\PosToAdd:\Seed.\Span\),\\
\(\WeightToAdd:r^\Rank=r^0=1\)\end{tabular}\})\)
\EndIf
\State return \(\Blocks\)
\end{algorithmic}
\end{algorithm}
The output of Algorithm~\ref{code:hashblock} is a vector of blocks, represented
...
the difference in number of \Qgram{q}s to be enumerated and the size of the
resulted score matrices.
\AVTcomment{clarify}
\begin{table}
\begin{center}
...
\(r=|\Alpha|\). This can be proven by induction on \(q\)~\cite{pfn}.
...
\(\Alpha_{i}\). This process, presented as a recursive function, is outlined in
...
given and based on the index tables \(\IT{i}{\Alpha}\) for all
\(i\), \(2\leq i\leq q\).
...
classes are outlined. The seed readers are based solely on the provided spaced 
seeds in compilation. The hash engine depends only on the reader and takes in 
additionally a sequence set at run time. The index tables have only the scoring 
function as an dependency and 
...
threshold at the beginning of the construction of \(k\)-environment.
...
approach~\ref{code:ctxsens},\ref{code:ctxsens1} to calculate
...
well against seed weight, therefore some testing was created to test for
...
\AVTcomment{In the case of sorted \Qgram{q}s, we need a table of size number of sorted \Qgram{q}s \(\times\) number of \Qgram{q}s}
\begin{figure}
\centering
