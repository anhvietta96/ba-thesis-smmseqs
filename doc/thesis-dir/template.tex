...
generated dataset consisting of \numprint{50000000} sequence characters. The
...
To examine the asymptotic behavior of the two hashing algorithms, seeds of 
weight 9 and 12 were further tested. For each seed weight \(q\), the number of 
seed blocks ranges from \([1,\frac{q}{2})\). The generated seeds are 
characterized in Table~\ref{tab:asymptoticseed9} and \ref{tab:asymptoticseed12}.
The test results were summarized in Figure~\ref{fig:asymptoticseed9} and 
...
algorithm only performs better when the seed has 1 or 2 blocks. In the case of 
seeds of weight 12, the hashing algorithm performs better than the normal 
approach. This signals that recursive hashing does asymtotically have a better 
run time, but with the caveat that better performance only occurs in the case 
where the seed weight is larger than twice the number of blocks 
\(q > 2b\), along with the fact that it's harder to implement and only offer 
marginal increase in performance, might mean that the normal hashing approach is
still the better general approach in hash value enumeration.
