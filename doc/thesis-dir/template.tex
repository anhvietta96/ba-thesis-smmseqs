...
The above described methods were implemented in a program called SMMseqs.
...
SuperMMseqs. The implementation in C++ followed an object oriented
approach,
makes heavy use of Metaprogramming and is based on the following
seven main classes:}
\begin{enumerate}
...
\SKcomment{seeds are not processed. They have been converted before. But
the
...
characters from the input sequences at positions specified by the space
seed.}
\item A recursive hash engine to collect hash values from sequences and
package them.
\SKcomment{The verb packages has not been used before. If you write 'every'
you cannot use 'hashes'. Please be more precise and consistent in your
wording.}
...
name B and
the public instance variables and methods. This does not necessariy
coincide
...
threshold estimator.}
\SKcomment{Please specify what a rhombus means and a rectangle means.
Likely
...
\SKcomment{Why use 'hash' in 'target hash data' but not in 'query data'?
After
all, the elements delivered by the environment constructor re hashed as
well.}
\begin{figure}[t]
\begin{center}
...
is referenced in the scoring function. The alphabet provides a rank
function
...
represented as a 64-bit integer constant,
\SKcomment{In the table in which you show the spaced seeds used, you do not
show the leading and trailing zeros. But if you represent a spaced seed as
a
constant, you have to have some convention. The most promiment is:
All spaced seed begin with a 1 and end with a 1. This would allow to
reprent
the spaced seed anywhere in a 64-bit unsigned integer. But you likely
represent
...
take in only scoring function as parameter and therefore, can be complete
be
...
the scoring function. They are based on the scoring function. Please be
more
precise.}
...
byte units and saved in a dynamically-sized contiguous container (i.e.\ a
vector).
In the processing of query sequences, firstly the \Qgram{2} and \Qgram{3}
score matrices \(\Scoretablename\) are evaluated using the index tables
\SKcomment{Why are the \Qgram{2} and \Qgram{3} matrices
\underline{evaluated}?
They have been computed at compile time, i.e.\ for each \(2\)-gram and
\(3\)-gram \(u\) one can enumerate \(2/3\)-gram/score-pairs
\((v,\sigma(u,v))\)
in sorted order.}
\AVTcomment{In my implementation they aren't evaluated at compile time due
to
stack issue, see Section~\ref{section:scorematrix}}
...
>>>>>>> 9d636de91660b29da28bd76580faca423b585cdd
...
\caption{Time measured in miliseconds to generate score
matrix between pairs of \Qgram{q}s}
