...
\newcommand{\Qsplit}{\mathsf{QS}_{q}}
...
as section heading. Also MMseqs2 is not an own chapter, but a section of
the
introduction.}
\AVTcomment{Fixed}
...
DIAMOND~\cite{buchfink2015fast},\cite{BUC:REU:DRO:2021} and
...
\AVTcomment{I found it in MMseqs paper.}
...
\AVTcomment{default workflow. Added.}
...
consecutive \Qgram{q}, and the new character respectively as a the
character
...
is
called the radix and \(i\) is the position of the character in the
\Qgram{q}.
...
\end{algorithmic}
\end{algorithm}
\SKcomment{I do not understand why you repeat this explanation and do not
just refer to the GSA-script.}
\SKcomment{While you provide a detailed description of existing methods,
your
innovation is only shortly explained. Please provide pseudocode for
the block based computation including the calculation of the custom radix
per block.}
...
number of blocks. The division schematic is illustrated in
...
\For{\(i \in [0,\Seed.\mathsf{span})\)}\Comment{Iterate over seed}
\If{\(\Seed[i] \land !\InBlock\)}\Comment{Start new block}
\State \(\InBlock \gets \True\)
\State \(Start \gets i\)
\Else
\If{\(!\Seed[i] \land \InBlock\)} \Comment{End current block}
\State \(\InBlock \gets \False\)
\State \(\Blocks.\mathsf{append}(\{\begin{tabular}{c}
\(\CharToRmv:\Start\),\\
\(\RankToRmv:r^{\Rank+i-1-\Start}\),\\
\(\CharToRmv:i\),\\
\(\RankToRmv:r^{\Rank}\)\end{tabular}\})\)
\EndIf
\EndIf
\If{\(\Seed[i]\)}
\State \(\Rank-=1\)
...
add
and rank to add. This preprocessing of spaced seeds runs in linear time
\(O(\Seed.\mathsf{span})\), same as the computation of first hash.
...
\end{algorithmic}
\end{algorithm}
A closer look into the algorithm shows that for every block, two operations
must
be performed, a subtraction and an addition, which means that the expected
run
time of the algorithm should be proportional to \(2nb\). Compared to the
method
...
The run
time is then expected to be equal when \(q=2b\) and \(\forall b <
\frac{q}{2},\)
...
Nr & Seed & Span & Weight \(q\) & Number of Blocks \(b\) & Speedup in
hashing
\(=\frac{q}{2b}\)\\
\hline
...
\end{align}
The \(k\)-environment of a \Qgram{q} \(u\) is then defined as~\cite{gsa}:
\begin{align}
\text{Env}_{k,\sigma}[u] = \{(v,\sigma(u,v))|v\in \Alpha^q,\sigma(u,v)\geq
k\}
\end{align}
In order to enumerate the \(k\)-environment, the most direct computation
method
would be to calculate a score table
\begin{align}
\Scoretable{q}{u}=\lbrack (v,\sigma(u,v))\mid v\in\Alpha^{q}\rbrack
\end{align}
where the score \Qgram{q} pairs are sorted after the score. The
\(k\)-enviroment
can then be extracted as the portion of \(\Scoretable{q}{u}\) where
\(\sigma\geq k\).
This approach is often infeasible for large \(q\) due to the exponential
complexity \(O(\Alpha^{2q})\) in both time and space. An alternative
approach
...
algorithm considers only single character \(u[i],0\leq i<q\) and
construction a
score table for each character \(\Scoretable{1}{u[i]}\). Then the
\(k\)-environment is contructed as a subset of the cartesian product
\(\bigtimes \Scoretable{|u_i|}{u_i}\):
\begin{align}
\text{Env}_k(u) = \{(v,\sigma(u,v))|(v,\sigma(u,v))\in \bigtimes
\Scoretable{|u_i|}{u_i},\sigma(u,v)\geq k\}\label{eq:singleCharCartesian}
\end{align}
\SKcomment{You should first refer to the notion of \(k\)-environments
in the GSA-lecture notes. Based on this, you explain the the
branch-and-bound
algorithm for single characters. This will then be generalized to the
block-based method.}
...
\Qgram{3}s
using a splitting function \(\Qsplit\). For every integer \(q\), the output
of
\(\Qsplit\) would have the following properties:
\begin{itemize}
\item The output is an array, which length is \(\frac{q}{3}+
\begin{cases}
1, \text{if } (q \text{ mod } 3 = 0)\\
0, \text{else}
\end{cases}
\)
\item \(\forall q_i \in \Qsplit(q), q_i \in \{2,3\}\)
\item \(q = \sum_{q_i \in \Qsplit(q)}\)
\item \(\Qsplit(q)\) is sorted in descending order.
\end{itemize}
The examplary output of \(\Qsplit\) for \(q\in \{4,5,6,7\}\) is outlined in
...
Upon applying the splitting function \(\Qsplit\), the original \Qgram{q}
\(u\)
gets divided into non-overlapping \Qgram{q_i'}s \(u_i\) where \(q_i' \in
\{2,3\}\).
For each of these segments, a score table \(\Scoretable{q_i'}{u_i}\) is
computed
and the \(k\)-environment can then be enumerated similarly to
Equation~\ref{eq:singleCharCartesian}
\begin{align}
\text{Env}_k(u) = \{(v,\sigma(u,v))|(v,\sigma(u,v))\in \bigtimes
\Scoretable{|u_i|}{u_i},\sigma(u,v)\geq k\}\label{ScoreTablesCartesian}
\end{align}
\SKcomment{Do not reuse the identifier \(u\) below, as it was introduced
as \(q\)-gram above. Also use \(q'\in\{2,3]\), as otherwise you cannot
refer to the original \(q\)-gram.}
\SKcomment{Why prioritize \(q'=3\)? Does this mean that for \(q=4\) you
split
it into a single character and a \(3\)-gram? You likely are only referring
to
the method for \(q\geq 5\).}
\AVTcomment{I meant that a 6-gram would get decomposed to 2 3-grams instead
of 3
2-grams. The wording is changed.}
\SKcomment{In the above, the notation \(u_{i}\) is not defined. What you
actually need to define first is the splitting of \(q\in\{4,\ldots,7\}\)
into
a sequence \(\Qsplit\) of ordered values from the set
\(\{2,3\}\), such that \(q=\sum_{q'\in\Qsplit}q'\). This gives the
following splitting dependent on \(q\in \{4,\ldots,7\):
\(4\mapsto [2,2]\),
\(5\mapsto [2,3]\),
\(6\mapsto [3,3]\),
\(7\mapsto [2,2,3]\).
On each \(q\)-gram
\(u\), \(\Qsplit\) induces an ordered splitting into substrings of the
lengths specified in \(\Qsplit\). Lets denote this splitting
as \(\Qsplit(u)\). Then state that the score table
\(\mathsf{ST}(q')\) is precomputed for all \(q'\in\Qsplit\). That is,
for an arbitrary but fixed \(q'\) and
for all \(u'\in\Alpha^{q'}\), \(\Scoretable{q'}{u'}\) is the sequence of
pairs
\((v,\sigma(u',v))\) for all \(v\in\Alpha^{q'}\) stored in
descending order of the score-values. Then one can compute
\(\text{Env}_k(u)\) as follows:
For all \(u'\in\Qsplit(u)\) in the given order one enumerates
the cartesian product of all
\((v,\sigma(u',v))\in\mathsf{ST}(q')\),
such that the sum of the scores of each
tuple is at least \(k\).}
\SKcomment{You should now provide an example with numbers of the size of
the
scoretables for \(q'\in\{2,3\}\) which need to be precomputed. I assume
that
you compute them without considering the score threshold \(k\) which only
has
an effect when enumerating the cartesian product.}
\AVTcomment{The table is added below to also support sorting method}
\SKcomment{Also provide an example showing that the score of two
\(q\)-grams
does not change if the same permutation is applied to both.}
\AVTcomment{Below.}
...
permutation
function, the \(i\)-th character gets shuffled to \(p[i]\).
\begin{figure}[t]
\begin{center}
...
\Qgram{q} type & \(q\) & Number of \Qgram{q} & Score matrix size\\
\hline
Sorted & 2 & 210 & \numprint{44100}\\
& 3 & \numprint{1540} & \numprint{2371600}\\
\hline
