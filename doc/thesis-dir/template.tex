...
\(q\)~\cite{pfn}. Since there are \(\binom{q+r-1}{q}\) score tables 
...
\AVTcomment{I'm not sure how to prove this, since the condition, that 
\(sc(u)=\sum_{p=0}^{q-1} w(p,\Subchar{u}{p})\) correctly points to the index 
of \(u\), is built in to Lemma~\ref{lm:weightproof}. Below I've added some 
justification for this.}

\(w(q',a)\) exists and is unique, therefore we can construct a table of size 
\(q\times \Alpha\) to cache all \(w(q',a)\) for all \(0 < q' \leq q\), 
\(a\in \Alpha\). To compute these weights, the index tables \(\IT{q}{\Alpha}\) 
...
index of \(u\), since the \(i\)-th iteration would guarantee that the all 
weights \(w(i,a)\), \(a\in\Alpha\) are correct. The function \(sc(u)\) is also 
always bijective, since its value for \(u\in \Alpha^q\) is constrained in 
\([0,|M|)\) by design of the index table.

Based on the above description, Algorithm~\ref{code:linearEncodingTable} 
outlines a recursive method to compute a \(q\times \Alpha\)-table
\(\LE{q}{\Alpha}\). The base case of \(q' = 1\) can be 
directly given and based on the index tables \(\IT{i}{\Alpha}\) for all
\(q'\), \(2\leq q'\leq q\).
the \Qgram{q'}s are tracked for change in prefix and the weight of the
prefix can be calculated with Equation~\ref{eq:weightcalc}. The
...
\AVTcomment{I've added some explanation above.}
...
In this chapter, we discuss on how we design a program that utilizes the above 
ideas and how that program was implemented in a practical programming system.

The methods described above were implemented with a object-oriented approach in 
a program called SMMseqs, which makes extensive use of the template system in 
C++, with intention on scalability and high level of generalization.
...
\AVTcomment{I've reworded last section.}
...
mentioned above.}
\AVTcomment{the data vectors are indeed sorted after the score. I've changed 
the wording in the last section to mention that.}
...
&corrected threshold \(k_c\)\\
&function \(\mathsf{createQgram}\ref{code:createq}\)\\
&invert permutation \(\varphi\)
\end{tabular}

\begin{tabular}{@{}l@{~}l}
\textbf{Note:}&notation \(vw(i)\) refers to \(v_i[\Wheel[i]]\)\\
\end{tabular}
\begin{algorithmic}
\State \(\Loopid \gets n-1\)
\State \(\Wheel \gets [0]\times n\)
\State \(\Score \gets \sum_{i=0}^{n-1} vw(i).\Score\)
\If{\(\Score < k\)} \Comment{Current data vectors not viable. Early
Termination.}
\State return
\Else \Comment{First loop iteration}
\State \(\mathsf{createQgram}(\varphi,vw(0),vw(1),\ldots,vw(n-1))\)
\EndIf
\While{True}
...
\Else
\State \(\Score \gets \sum_{i=0}^{n-1} vw(i).\Score\)
\If{\(\Score \geq k_c\)}\Comment{Viable \Qgram{q} found}
\State \(\mathsf{createQgram}(\varphi,vw(0),vw(1),\ldots,vw(n-1))\)
...
\begin{algorithm}[t]
\caption{create \Qgram{q} from score-code pairs}
\label{code:createq}
\begin{tabular}{@{}l@{~}l}
\textbf{Input:}&\Qgram{q'} index table \(\mathsf{QIT}_{q'}\) for \(q'\in\{2,3\}\)\\
&score-code pairs \(p_0\), \(p_1\), ..., \(p_{n-1}\)\\
&invert permutation \(\varphi\)
\end{tabular}

\begin{tabular}{@{}l@{~}l}
\textbf{Note:}&notation \(u_i\) refers to \(\mathsf{QIT}_{\Qsplit(q)[i]}[p_i.\Code]\)\\
\end{tabular}
\begin{algorithmic}
\State \(u \gets u_0u_1\ldots u_{n-1}\)\Comment{Concatenate individual \Qgram{q'}s corresponding to each code}
\State \(u \gets \varphi(u)\)
\end{algorithmic}
\end{algorithm}
...
\begin{comment}
The advantage of this method however comes in more compact memory usage, which 
should be taken advantage of in the representation of the score matrices. Based 
on Table~\ref{tab:numdiff}, we could see that for the 210 sorted \(2\)-grams,
8 bits of encoding are needed, compared to 9 bits for the \(2\)-grams.
For the 1540 sorted \(3\)-grams, 11 bits of encoding are needed, compared to 13 
bits for the \(3\)-grams. That means that if the score fits in 5 bits, a 
\end{comment}
...
highest sensitivity of \numprint{1000} were used.
For the seed of weight 8, the program takes \numprint{4004}~ms to generate 
\numprint{204688068} \Qgram{8}s. This averages to 19.5~ns to generate each 
\Qgram{8}, which stays close to the average time of \Qgram{7} generation 
(20.1~ms, 3~\% less). For the seed of weight 9, the program takes 
\numprint{7446}~ms to generate \numprint{337761491}  \Qgram{9}s. This averages 
to 22.0~ns to generate each \Qgram{9}, which stays close to the average time of 
\Qgram{7} generation (20.1~ms, 10~\% more). These two tests present promising 
results and show that the construction of \(k\)-environment for \Qgram{8} and 
\Qgram{9} is both viable in run time performance.

Another interesting result shown in the tests is that the average number of 
\Qgram{9}s generated per position using context-free model 
\(\frac{\numprint{337761491}}{\numprint{210848}}\approx \numprint{1602}\) is 
60.2~\% larger than expected (\numprint{1000}) while the context-sensitive model
gives a more accurate approximation of 
\(\frac{\numprint{195296133}}{\numprint{210848}}\approx 926\), which 
corresponds to only 7.6~\% smaller than expected. This shows that for larger 
\(q\), the context-sensitive approach could show a lot more stable results. The 
run time of this approach can also be dramatically reduced if 
the score matrices of \Qgram{q} pairs are constructed in compile time, and 
therefore the value of this method should be investigated further.
