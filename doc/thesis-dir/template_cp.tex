...
significant
structure likeness or a connection to an intermediate sequence. There rises
a
...
developing searching and clustering methods that can handle large-scale datasets
efficiently.
...
Since the database can be very large, a precise and efficient enumeration of 
...
and
new characters respectively, which are both determined by the spaced seed. If 
the spaced seed can be maximally divided into \(b\) blocks of consecutive 1s, 
the \(b\) old characters can be characterized as the start of each block, and 
the new character respectively as the character directly after the end of the 
...
functions. I fixed it by rewording the section, naming recursive hash function 
...
\State \(\Blocks.\mathsf{append}(\{\begin{tabular}{c}
\(\PosToRmv:\Start\),\\
\(\RankToRmv:r^{\Rank+\Seed.\mathsf{span}-1-\Start}=r^{\Seed.\mathsf{span}-1-\Start}\),\\
\(\PosToAdd:\Seed.\mathsf{span}\),\\
...
clearer through a struct. The notation is just to clarify each number in the 
tuple.}
...
\For{\(i \in [0,\Seed.\mathsf{span})\)}\Comment{Iterate over seed}
\If{\(\Seed[i]\)}
\State \(\HashValue \text{ \times= } r\)
\State \(\HashValue \text{ += } s[i]\)
\EndIf
\EndFor
\State \(\HashValVec .\Append(\HashValue)\)
\For{\(j\) \textbf{from} 1 to $n-q$}\Comment{Compute other hash values}
\For{\(\Block \in \Blocks\)}
\State \(\HashValue \text{ -= } \Block.\RankToRmv \cdot s[\Block.\PosToRmv + j]\)
\label{code:lookup1}
\EndFor
\State \(\HashValue \text{ \times= } r\)\label{code:extraMultiplication}
\For{\(\Block \in \Blocks\)}
\State \(\HashValue \text{ += } \Block.\RankToAdd \cdot s[\Block.\PosToAdd + j]\)
\label{code:lookup2}
\EndFor
\State \(\HashValVec.\Append(\HashValue)\)
\EndFor
...
In Algorithm~\ref{code:invintseed}, for each block, two lookup operations must
be performed, along with three additions, one subtraction and two 
multiplications. There is also an extra intermediate multiplication. The method 
depicted in Algorithm~\ref{code:mmseqshash} in turn only requires for each 
character one lookup, two additions and one multiplication. Under the assumption
that subtraction and addition takes the same ammount of time, and the single 
...
\(\frac{q}{2b}\). In Table~\ref{tab:seeds}, the default seeds used in MMseqs2
...
No & Seed & Span & Weight \(q\) & Number of Blocks \(b\) & Speedup factor\\
&&&&&\(=\frac{q}{2b}\)\\
\hline
1& \numprint{11101} & 5 & 4 & 2 & 1 \\
2& \numprint{111011} & 6 & 5 & 2 & 1.25\\
3& \numprint{1101011} & 7 & 5 & 3 & 0.83 \\
4& \numprint{110010000101} & 12 & 5 & 4 & 0.625\\
5& \numprint{11101101} & 8 & 6 & 3 & 1\\
6& \numprint{1101010011} & 10 & 6 & 4 & 0.75\\
7& \numprint{1111010101} & 10 & 7 & 5 & 0.7\\
8& \numprint{11010110011} & 11 & 7 & 4 & 0.875\\
\end{tabular}
\caption{Seeds used in MMseqs2 with their span, weight, number of blocks
and
theoretical hashing speedup using recursive hashing\label{tab:seeds}}
\end{center}
\end{table}
...
\AVTcomment{I use 1 here beause it refers to \Qgram{1}s. For index in loops and
...
Environment of \Qgram{3}s through computation of 3x3 score matrix) and 2. Single
...
& ~s:=\sum\nolimits_{i=0}^{|\Qsplit(|u|)|-1}s_{i}\geq k\}\label{ScoreTablesCartesian}
\end{align}
...
in
Figure~\ref{fig:shuffle}. The premutation function is internally represented as
an
...
\Qgram{q} type & \(q\) & Number & Score matrix & Matrix size ratio\\
& & of \Qgram{q}s &\(m\) size &\(\frac{|m_\text{sorted \Qgram{q}}|}{|m_\text{unsorted \Qgram{q}}|}\)\\
\hline
2 & Sorted & 210 & \numprint{44100} & 0.275625\\
& Unsorted & 400 & \numprint{160000}& \\
\hline
3 & Sorted & \numprint{1540} & \numprint{2371600} & 0.3705625\\
& Unsorted & \numprint{8000} & \numprint{6400000} &\\
\end{tabular}
...
\(q\) can be computed as the binomial \(\binom{q+r-1}{q}\) where \(r=|\Alpha!\).
This can be proven by induction on \(q\).

%%%TODO:Proof

An index table of sorted \Qgram{q}s \(\IT{q}{\Alpha}\) can be computed 
recursively.
...
\Qgram{q'} for \(q'=5-2=3\) and for the alphabet \(\Alpha_1=\{a_1,a_2\}\).
This idea can be generalized as follows:

Consider the construction of a sorted \Qgram{q} \(u\) of which the prefix of
\(m\leq q\) characters has been determined and \(\Subchar{u}{m-1}=a_{i}\) for
some \(i\), \(0\leq i\leq r-1\). Then
the task is to enumerate all sorted \Qgram{(q-m)}s over the alphabet
\(\Alpha_{i}=\{a_{i},a_{i+1},\ldots,a_{r-1}\}\).
...
slice notation to denote the list of remaining characters for iterate over in the for-loop. Also the indextable can be considered a global variable. As part
...
\(c:M\to\Nats_{0}\) defined by
\begin{align}
c(u)=\sum_{p=0}^{q-1} w(p,\Subchar{u}{p})\label{Equation:linearenc}
\end{align}
satisfies
\begin{itemize}
\item \(c(u)\in\{0,\ldots,|M|-1\}\) for all \(u\in M\) and
\item \(c(u)\neq c(u')\) for all \(u,u'\in M\), \(u\neq u'\).
\end{itemize}
\end{Lemma}

The recursive method to compute a \(q\times r\)-matrix
LE is then derived from the proof above. The base case can be directly given
...
\end{tabular}
\begin{algorithmic}
\State \(\LE{q}{\Alpha}\) \(\gets [[0,...,|\Alpha|-1]]\)
\For{\(q_i\in[2,q]\)}
\State \(\CurrentWeight \gets [\bot \times |\Alpha|]\)
\For{\(i,u\in\text{IT}_{q_i,\Alpha}\)}\Comment{Key-Value loop}
\If{\(\CurrentWeight [u[0]] = \bot\)}
\State \(\SuffixCode\gets 0\)
\For{\(j \in [1,q_i]\)}
\State \(\SuffixCode\text{ += }\LE{q}{\Alpha}[j-1][u[j]]\)
\EndFor
...
\SKcomment{Use \(r\) for the
alphabet size and \(\bot\) instead of \(\mathsf{None}\).}
\SKcomment{ Instead of the inner
for-lop you should use a \(\sum_{j=0}^{q-1}\). Please be consistent with
respect to indexing a sequence. You had mostly use 0-indexing before, but here
you switch to 1-indexing.}

\SKcomment{In Algorithm  \ref{code:linearEncodingTable} you only update
\(\CurrentWeight\) at index \(\Subchar{u}{i}\), which is likely not correct.}
...
of \(\CurrentWeight\) to \(\LE{q}{\Alpha}\). This should somehow be commented.}
...
a \(z\)-score statistics was applied. For each query sequence,
a calibration search through a subset of \numprint{100000} randomly
sampled target sequences

\SKcomment{Sampled from what set?}

\SKcomment{I recommend to not index the sequences and just denote a sample as
a subset \(S'\) of a sequence set \(S\). And you should always make the
value on the left depend on what it depends on,
i.e.\ \(S'\) needs to appear on the left hand side.}

is performed, where the number of prefiltering operations is
\begin{align}
\text{sum}_L = \sum_{t=1}^{100~000} (L_t-k+1)
\end{align}

\SKcomment{Then you can simply state the
\(L(S')=\sum_{s\in S'}(\max(0,|s|-\mathit{span}+1)\) is the total number of
\(q\)-mers to extract using a spaced seed of the given span.}
...
\SKcomment{Unclear what you mean by prefiltering score. Where was it defined?}
\SKcomment{To perform prefiltering,
you have to have a threshold, but this is what you want to determine here. I
am lost.}
\SKcomment{Also \(t\) is the index of a sequence, but you denote it as
a target sequence. Please be more precise.}

\(L_t\) here denotes the length of the target sequence \(t\)
...
\SKcomment{What is \(t\) here? A single target sequence? }

Assuming the number of \Qgram{q} matches

\SKcomment{\Qgram{q} matches of which sequences with which sequences? Do
you mean exact \(q\)-gram matches or \(q\)-grams whose score is above the
score threshold.}

is Poisson-distributed, the
standard deviation of the scores \(\sigma_S\)

\SKcomment{Why does this assumption make sense. Please provide arguments or
citations.}

\SKcomment{What is \(\Sigma_{S}\)? Or did you want to write: the
standard deviation \(\sigma_S\) of the score distribution.}

can be computed through
number of expected \Qgram{q} matches \(n_{\text{match}}\):

\SKcomment{You likely mean the 'expected number
\(n_{\text{match}}\) of \Qgram{q} matches'. You really need to be more
precise with your wording.}
...
\SKcomment{Sorry I am lost here. What is a chance \Qgram{q} match?}

\begin{align}
S_{qt} \geq Z_{thr}\sigma_S + S_0
\end{align}
where \(Z_{thr}\) is the significant \(z\)-score.

\SKcomment{What is the significant \(z\)-score? Please explain.}

MMseqs2 would take in a
sensitivity parameter \(s\), labeled internally as the average length of the
\Qgram{q} list at each sequence position,

\SKcomment{The \(q\)-list at each sequence position should be the size of the
\(k\)-environment.}

from the user and using a heuristic
to compute \(S_{qt}\). This approach is shown to be lacking control
for \(s\)

\SKcomment{What is \(s\)?}
\SKcomment{Does 'is shown' above mean that you have shown that it does not
allow to control \(s\)? I assume since otherwise you would have written
'will be shown'. But you actully want to gain control of the
\(k\)-mer environment.}

\SKcomment{A more direct approach would be to start with a small value for the
score threshold, compute the \(k\)-environment and extend it until one
has achieved an average size as defined beforehand. Even better would be
the enumerate the elements of the environment in descending order of the score.
This would allow to enumerate, until the appropriate size is reached.}

and therefore the goal is to streamline the process and to allow
for a more fine-grained control of the length of \Qgram{q} list.

\SKcomment{As you can see from the number of comments, I think that this section
needs to be rewritten, as to allow to understand it.}

\subsection{A context-free approach}

\SKcomment{Unclear to what context you refer.}

The average \Qgram{q} list \(l\) and the

\SKcomment{Do not use the identifier \(l\), as this can hardly be distinguished from \(1\). Use \(\ell\) instead.}

threshold \(k\) of the environment can be related based on the tail
of a discrete distribution of scores for pairs of \(q\)-grams. So let
\(h_{q}(x)\) be the number of pairs
\((u,u')\in\Alpha^{q}\times\Alpha^{q}\) such that \(\sigma(u,u')=x\).

\SKcomment{Please check that this the correct definition of
\(h_{q}\). But you actually have plotted this only for a sample, but I assumed
...
added a Python-script \texttt{bin/pair\_score\_distrib.py}. Results look okay,
...
distributed pairs of \Qgram{7} using BLOSUM62 score matrix. The blue bars show 
...
\SKcomment{You likely mean 'uniformly
distributed pairs of \(q\)-grams. It is not an example. You likely have
determine sample \(Q\) of \(q\)-grams for \(q=7\) and then computed the score
for each \((u,u')\in Q\times Q\), \(u<u'\)? How large was \(Q\)? Did you specify which score function you use?}
...
Under the assumption that individual sequence characters follow a context-free 
model, i.e they occur independently from what comes before and after, the 
\(q\times q\) score distribution can be approximated recursively
through convolution:
\begin{align}
h_{q}(x) = \sum_{y\in\mathcal{Z}} h_{q-1}(y)h_1(x-y)
\label{equation:convolution}
\end{align}
It is important to note that \(h_1\), the 1-gram score distribution, depends 
on both the character distribution collected from target sequence and the
uniform character distribution, since the characters appeared in the
environment are equally likely to occur. The pseudocode for this computation is
summarized in Algorithm~\ref{code:ctxfree}.

\begin{algorithm}[t]
\caption{Approximation of an appropriate threshold \(k\) in context-free approach}
\label{code:ctxfree}
\begin{tabular}{@{}l@{~}l}
\textbf{Input:}&single character distribution \(d\)\\
&score function \(\sigma\) with smallest and biggest score \(\Small\) and \(\Large\)
respectively\\
&\Qgram{q} length \(q\)\\
...
\end{tabular}
\begin{algorithmic}
\State \(\PossibleValuesSingle \gets (\Large - \Small + 1)\)
\State \(\PossibleValuesTotal \gets \PossibleValuesSingle\times q\)
\State \(h_1 \gets [0 \times \PossibleValuesTotal]\)
\For{\(c_1 \in \Alpha\)}
\For{\(c_2 \in \Alpha\)}
\State \(h_1[\sigma(c_1,c_2)-\Small]\text{ += }d[c_1]/r\)
\EndFor
\EndFor
\State \(h \gets h_1\)
...
\For{\(i \in [0,\PossibleValuesSingle\times n)\)}
\For{\(j \in [0,\PossibleValuesSingle)\)}
\State \(h_{\text{temp}}[i+j]\text{ += }h[i]\times h_1[j]\)
\EndFor
\EndFor
...
processing of target dabase. This approach is but often complicated in
large \(q\), since a \(q\times q\) score matrix must be computed, which
incurs an additional penalty of \(O(|\Alpha|^{2q})\), which is often infeasible 
when \(q>4\). A middle ground is to divide the \Qgram{q}s
into two or three shorter segments, possibly similar to the branch and bound 
algorithm in Section~\ref{section:qgramgen}, and collect their positional 
distribution separately, i.e the number of distributions corresponds directly to
number of segments, regardless of their length. These distributions can then be 
convoluted together to approximate the \Qgram{q} distribution. The extraction of
threshold \(k\) then follows the same approach in the context-free variant.The 
pseudocode for this computation is
summarized in Algorithm~\ref{code:ctxsens}.

\begin{algorithm}[t]
\caption{Approximation of an appropriate threshold \(k\) in context-sensitive 
approach}
\label{code:ctxsens}
\begin{tabular}{@{}l@{~}l}
...
\For{\(i\in[0,|\Qsplit(q)|)\)}\Comment{Create factors to extract hash value of small segments}
\State \(\ModFactors.\Append(r^{\Qsplit(q)[i]})\)
\State \(\DivFactor \gets 1\)
\For{\(j\in[i+1,|\Qsplit(q)|)\)}
\State \(\DivFactor\text{ \times= }\Qsplit(q)[j]\)
\EndFor
...
\For{\(i\in[0,|\Qsplit(q)|)\)}
\State \(\HashValue_i \gets (\frac{\HashValue}{\DivFactors[i]}) \% \ModFactors[i]\)\Comment{Integer division}
...
\State \(h_b[i][m_{\Qsplit(q)[i]}[\HashValue_1][\HashValue_2]]\text{ += }\frac{d[i]}{\ModFactors[i]}\)
\EndFor
\EndFor
\EndFor
...
\For{\(n \in [1,|\Qsplit(q)|)\)}\Comment{Convolute \(|\Qsplit(q)|-1\) times}
\State \(h_{\text{temp}} \gets [0 \times \PossibleValuesTotal]\)
\For{\(i \in [0,\PossibleValuesSingle\times \ell)\)}
\For{\(j \in [0,\PossibleValuesSingle\times \Qsplit(q)[n])\)}
\State \(h_{\text{temp}}[i+j]\text{ += }h[i]\times h_b[n][j]\)
\EndFor
\EndFor
...
BLOSUM62
score matrix.
\section{Enumeration of hash values}
...
generated dataset consisting of \numprint{50000000} sequence positions. The
