...
\label{code:lookup1}
\EndFor
\State \(\HashValue \text{ *= } r\)\label{code:extraMultiplication}
\For{\(\Block \in \Blocks\)}
...
\caption{Ratio of number of generated \Qgram{q}s using MMseqs2 \(z\)-score 
statistics approach vs. expected. Data were collected using default MMseqs2
configuration on test datasets (See Chapter~\ref{chapter:benchmark}).}
...
stage.
\Chapter{Benchmark and Performance\label{chapter:benchmark}}
...
MMseqs2
result (see Figure~\ref{fig:mmseqssens}).
\begin{figure}
\centering
